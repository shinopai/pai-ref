...
<div class="flex flex-wrap justify-center items-start">
    ...
    <div class="item w-2/3 h-auto">
        ...
        <div class="container mx-auto mt-5 bg-white py-5 px-2">
            ...
            <div class="flex flex-wrap justify-start items-center p-2">
                ...
                <div class="item w-1/3 h-auto p-1"><span class="material-icons" style="vertical-align: -5px;">
                        arrow_right
                    </span><a href="#" class="hover:text-blue-500">{{ $category->name }}({{ $category->photos->count() }})</a></div>
                @endforeach
            </div>
        </div>
        ...
        <!-- search result -->
        <div class="container mx-auto mt-5 bg-white py-5 px-2">
            <h2 class="bg-gray-200 text-xl font-bold w-full p-2">検索結果(カテゴリー:「{{ $category_name }}」/撮影地:「{{ $location_name }}」/キーワード:「{{ $search_keyword }}」)</h2>
            ...
            @else
            <p class="mt-3">写真が見つかりませんでした</p>
            @endif
            {{ $photos->appends(request()->input())->links('vendor.pagination.default') }}
